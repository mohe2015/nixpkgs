...
- `yarn2nix-moretea.spdxLicense` is now renamed to `yarn2nix-moretea.getLicenseFromSpdxId`,
and is now derived from the new `lib.meta.getLicenseFromSpdxId`. The renamed function is only referenced by `yarn2nix-moretea.mkYarnPackage` in Nixpkgs so far.
