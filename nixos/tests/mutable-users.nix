...
import ./make-test-python.nix ({ pkgs, lib, ...} : {
  ...
  # TODO add a test that creates the max amount of users so all uids are used.
  # then in the second step delete all the users (declaratively)
  # and in the third step try to create some other users
  # and see which uids they get or if it errors out
  nodes = let
    maybeOutOfUids = i: { ... }: {
  ...
  in ''
    ...
    assert "whichuid:x:" in notOutOfUidsNoReuse.succeed("cat /etc/passwd")
