...
let
  ...
  hackage2nix = name: version:
    let component = all-cabal-hashes-component name version; in
    self.haskellSrc2nix {
      name = "${name}-${version}";
...
package-set { inherit pkgs lib callPackage; } self // {
    ...
    let drv =
      (extensible-self.extend
        (pkgs.lib.composeExtensions
          ...
          overrides)).callCabal2nixWithOptions name
        root
        cabal2nixOptions
        { };
    in
  ...
  shellFor =
    ...
    in
    pkgWithCombinedDepsDevDrv.overrideAttrs (old: mkDerivationArgs // {
